...
class CloudMailBusinessError(RuntimeError):
    def __init__(self, code: object, message: str):
        super().__init__(f"CloudMail business error: {message}")
        self.code = str(code)
...
class CloudMailClient:
    ...
    def _post_json(self, path: str, payload: Dict, require_token: bool):
        try:
            return self._post_json_once(path, payload, require_token)
        except CloudMailBusinessError as error:
            if not require_token or not is_token_validation_error(error):
                raise
            self._token = ""
            return self._post_json_once(path, payload, require_token)

    def _post_json_once(self, path: str, payload: Dict, require_token: bool):
        ...
        if code is not None and int(code) != 200:
            ...
            raise CloudMailBusinessError(code, message)
        return data
...
def is_token_validation_error(error: CloudMailBusinessError) -> bool:
    text = f"{error.code} {error.message}".strip().lower()
    return any(marker in text for marker in TOKEN_VALIDATION_MARKERS)
